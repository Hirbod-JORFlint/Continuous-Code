...
(cd $OPC_ROOT && uv run python -m runtime.harness scripts/mcp/nia_docs.py \
  --package "$LIBRARY" \
  --registry "$REGISTRY" \
  --query "$TOPIC" \
  --limit 10)
...
(cd $OPC_ROOT && uv run python -m runtime.harness scripts/mcp/firecrawl_scrape.py \
...
(cd $OPC_ROOT && uv run python scripts/mcp/perplexity_search.py \
