---
description: Extract perception changes from session thinking blocks and store as
  learnings
mode: subagent
permission:
  '*': deny
...
(cd $OPC_PROJECT_DIR/opc && uv run python scripts/core/extract_thinking_blocks.py \
  --jsonl "$JSONL_PATH" \
  --filter \
...
(cd $OPC_PROJECT_DIR/opc && uv run python scripts/core/store_learning.py \
