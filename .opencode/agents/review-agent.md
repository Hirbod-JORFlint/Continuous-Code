---
description: Review implementation by comparing plan (intent) vs Braintrust session
  (reality) vs git diff (changes)
mode: subagent
---
...
```bash
# Find today's plans
ls -la $OPC_PROJECT_DIR/thoughts/shared/plans/

# Or check the ledger for the current plan
grep -A5 "Plan:" $OPC_PROJECT_DIR/CONTINUITY_*.md
```
