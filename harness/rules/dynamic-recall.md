...
```bash
(cd $OPC_ROOT && PYTHONPATH=. uv run python scripts/core/recall_learnings.py --query "your search terms")
```
...
(cd $OPC_ROOT && PYTHONPATH=. uv run python scripts/core/recall_learnings.py --query "authentication patterns")

# More results
(cd $OPC_ROOT && PYTHONPATH=. uv run python scripts/core/recall_learnings.py --query "error handling" --k 10)

# Pure vector search (higher similarity scores, 0.4-0.6 range)
(cd $OPC_ROOT && PYTHONPATH=. uv run python scripts/core/recall_learnings.py --query "database schema" --vector-only)

# Text-only search (fast, no embeddings)
(cd $OPC_ROOT && PYTHONPATH=. uv run python scripts/core/recall_learnings.py --query "YAML format" --text-only)
```
